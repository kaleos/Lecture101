...
test('Place the order', async ({ page }) => {
  ...
  for(let i=0; i<await rows.count(); ++i)                                           // Similar loop but this time with await and .count
  {
      ...
      if (response.orderId.includes(rowOrderId))                                             // This is asking if orderId on line 63 matches rowOrderId 
      {
  ...
  expect(response.orderId.includes(orderIdDetails)).toBeTruthy();                                            // The orginal ID from line 63 matching orderIdDetails from line 79.
});
